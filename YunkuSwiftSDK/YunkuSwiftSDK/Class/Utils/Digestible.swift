...
public protocol Digestible {
    func withDigestibleData(_ append: (UnsafeBufferPointer<UInt8>) -> Void)
}

extension Data: Digestible {
    /// NSData objects can be added to a digest. The digest will include the bytes in 
    /// the NSData object.
    public func withDigestibleData(_ append: (UnsafeBufferPointer<UInt8>) -> Void) {
...
internal struct DigestibleArray: Digestible {
    var array: [UInt8]
    internal func withDigestibleData(_ append: (UnsafeBufferPointer<UInt8>) -> Void) {
